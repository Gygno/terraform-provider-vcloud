...
description: |-
  Provides a resource to manage Viettel IDC Tenant Manager Organization VDC.
---
...
This resource is exclusive to **Viettel IDC Tenant Manager**. Supported in provider
*v4.0+*
...
data "vcloud_tm_supervisor" "supervisor" {
  name       = "my-supervisor"
  vcenter_id = vcloud_vcenter.vc.id
}
...
data "vcloud_tm_region_zone" "one" {
  region_id = data.vcloud_tm_region.one.id
  name      = "my-zone"
}

resource "vcloud_tm_org_vdc" "first" {
  name           = "my-org-vdc"
  org_id         = vcloud_tm_org.test.id
  region_id      = data.vcloud_tm_region.one.id
  supervisor_ids = [data.vcloud_tm_supervisor.test.id]
  zone_resource_allocations {
    region_zone_id         = data.vcloud_tm_region_zone.one.id
    cpu_limit_mhz          = 2000
...
* `supervisor_ids` - (Required) A set of Supervisor IDs that back this Org VDC. Can be looked up
  using [`vcloud_tm_supervisor`](/providers/viettelidc-provider/vcloud/latest/docs/data-sources/tm_supervisor) data source
...
* `region_zone_id` - (Required) Can be looked up using
  [`vcloud_tm_region_zone`](/providers/viettelidc-provider/vcloud/latest/docs/data-sources/tm_region_zone) data source
...
```
terraform import vcloud_tm_org_vdc.imported my-org-name.my-org-vdc-name
```
